...
def replace_batchnorm(model):
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, torch.nn.BatchNorm2d):
            model._modules[name] = torch.nn.GroupNorm(1, module.num_features)
...
model.to(device)
model.load_state_dict(torch.load('model_and_optimizer_140.pth')['model_state_dict'])
model.segmentation_head[0] = nn.Conv2d(16, 4, kernel_size=(3, 3), padding=(1,1)).to(device)
