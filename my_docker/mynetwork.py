...
class MyNetWork():
    ...
    def __init__(self, load_path=None, load_path_tubulin=None, load_path_actin=None):
        self.model = smp.Unet(
        ...
        self.model_tubulin.to(self.device)    
        self.model_tubulin.load_state_dict(torch.load(load_path_tubulin)['model_state_dict'])

        self.model_actin = smp.Unet(
        encoder_name="efficientnet-b7", # choose encoder, e.g. mobilenet_v2 or efficientnet-b7
        encoder_weights=None, # use `imagenet` pre-trained weights for encoder initialization
        in_channels=1, # model input channels (1 for gray-scale images, 3 for RGB, etc.)
        classes=4, # model output channels (number of classes in your dataset)
        ...
        self.model_actin.to(self.device)    
        self.model_actin.load_state_dict(torch.load(load_path_actin)['model_state_dict'])

        
        
        
        self.model.eval()
        self.model_tubulin.eval()
        self.model_actin.eval()
    ...
    def forward(self, image, tl):
        with torch.no_grad():
            ...
            out_tubulin = self.model_tubulin(image.unsqueeze(0))
            out_tubulin = torch.clip(out_tubulin[:,:,:o_shape[1],:o_shape[2]], 0, 1)
            out_actin = self.model_actin(image.unsqueeze(0))
            out_actin = torch.clip(out_actin[:,:,:o_shape[1],:o_shape[2]], 0, 1)
        print(out.shape, out.min(), out.max())
        results = []
        #for i in range(out.shape[1]):
        results.append(out[0,0].cpu().numpy())
        results.append(out[0,1].cpu().numpy())
        results.append(out_tubulin[0,2].cpu().numpy())
        results.append(out_actin[0,3].cpu().numpy())
        return results
